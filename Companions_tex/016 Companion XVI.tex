...
\subsection{Mirror-Sector Sound Speed in the Dark-Matter Perturbation Equations}
\label{sec:mirror-sound-speed-class}

As shown in Companion~XIII, the dark matter component in RDCC corresponds to the
baryonic matter of the CPT-conjugate mirror sector. Before mirror recombination, this
component forms a tightly coupled photon--baryon fluid with a finite, temperature-dependent
sound speed. Consequently, the dark matter cannot be treated as a pressureless fluid during
this epoch. The perturbation equations must therefore include a nonzero sound-speed term
$c_{s,\mathrm{mirror}}^{2}(a)$ until the mirror-sector recombination scale factor
$a_{\mathrm{rec,mirror}}$.

\subsubsection{Mirror-Sector Sound Speed}

The sound speed of the mirror photon--baryon fluid is given by
\begin{equation}
...
\label{eq:cs-mirror}
\end{equation}
where $\rho_{b,\mathrm{mirror}}$ and $\rho_{\gamma,\mathrm{mirror}}$ denote the mirror baryon and
mirror photon energy densities. After mirror recombination,
\begin{equation}
c_{s,\mathrm{mirror}}^{2}(a > a_{\mathrm{rec,mirror}}) \approx 0.
\end{equation}

\subsubsection{Modification of the Dark-Matter Continuity Equation}

The continuity equation for the mirror-sector dark matter becomes
\begin{equation}
\dot{\delta}_{\mathrm{DM}}
+
(1+w_{\mathrm{DM}})\left(\theta_{\mathrm{DM}} - 3\dot{\Phi}\right)
+
3\mathcal{H}\left(c_{s,\mathrm{mirror}}^{2} - w_{\mathrm{DM}}\right)\delta_{\mathrm{DM}}
=
- \Gamma \left(\delta_{\mathrm{DM}} - \delta_{A}\right),
\label{eq:dm-continuity}
\end{equation}
where $w_{\mathrm{DM}} \approx 0$ after mirror recombination and $\Gamma$ is the relaxation rate.

\subsubsection{Modification of the Dark-Matter Euler Equation}

The Euler equation becomes
\begin{equation}
\dot{\theta}_{\mathrm{DM}}
+
\mathcal{H}\left(1 - 3c_{s,\mathrm{mirror}}^{2}\right)\theta_{\mathrm{DM}}
-
k^{2}c_{s,\mathrm{mirror}}^{2}\,\delta_{\mathrm{DM}}
+
k^{2}\Psi
=
- \Gamma\left(\theta_{\mathrm{DM}} - \theta_{A}\right).
\label{eq:dm-euler}
\end{equation}

\subsubsection{Implementation in CLASS}

In the CLASS implementation (see Appendix~B), the mirror-sector dark matter is treated as
a fluid with sound speed $c_{s,\mathrm{mirror}}^{2}(a)$ until $a_{\mathrm{rec,mirror}}$. The following
modifications are required:

\begin{itemize}
\item Add $c_{s,\mathrm{mirror}}^{2}(a)$ to the background module.
\item Modify the perturbation module to include Eqs.~\eqref{eq:dm-continuity} and
\eqref{eq:dm-euler}.
\item Introduce a smooth transition to $c_{s,\mathrm{mirror}}^{2}=0$ at $a_{\mathrm{rec,mirror}}$.
\item Ensure numerical stability by treating the $k^{2}c_{s}^{2}$ term implicitly for large $k$.
\end{itemize}

This implementation ensures that the RDCC Boltzmann Code correctly captures the acoustic
suppression of small-scale power induced by the mirror-sector thermodynamics, as described in
Companion~XIII.
