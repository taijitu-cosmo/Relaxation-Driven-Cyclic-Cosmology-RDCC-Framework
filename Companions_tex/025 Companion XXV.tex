...
low-mass halos modify each of these processes, producing a distinctive global 21-cm
signature.
...
\label{eq:deltaT21}
\end{equation}
where $\delta_{\mathrm{b}}$ is the baryon overdensity, $\delta_{x_{\mathrm{HI}}}$ the neutral fraction
fluctuation, $\delta_{\mathrm{vel}}$ the velocity-gradient term, and $\delta_{T_{\mathrm{s}}}$ the
spin-temperature fluctuation.
...
P_{\mathrm{m}}(k,z),
\end{equation}
with $\mu = \hat{\mathbf{k}}\cdot\hat{\mathbf{n}}$.
...
\label{eq:BSD}
\end{equation}
with $R_{\mathrm{min}} \sim 0.5\,R_{\mathrm{ion,RDCC}}$ and $\beta \sim 2$.
...
width, and morphology of the global 21-cm absorption trough and the subsequent emission
era. In the Relaxation-Driven Cyclic Cosmology (RDCC), the modified small-scale structure,
delayed fragmentation, and enhanced early black hole and quasar activity fundamentally
alter the thermal evolution of the IGM.
...
\label{eq:EXRDCC}
\end{equation}
with $\chi_{X,\mathrm{src}} \sim 0.1$--$0.2$.
...
(h\nu - E_{\mathrm{ion}}),
\end{equation}
where $J_{\nu}$ is the specific intensity and $\sigma_{\nu}$ the photoionization cross section.
...
\label{eq:Ts}
\end{equation}
where $x_{\alpha}$ is the Lyman-$\alpha$ coupling coefficient and $x_{c}$ the collisional
...
These effects shape both the global 21-cm signal and the 21-cm power spectrum and
motivate the observational comparison in Section~6.
...
the Relaxation-Driven Cyclic Cosmology (RDCC). Building on the early-galaxy framework
of Companion~XXII, the black hole formation and growth model of Companion~XXIII, and
the quasar radiative feedback analysis of Companion~XXIV, we have shown that the
relaxation-modified dynamics of RDCC fundamentally reshape the thermal and ionization
