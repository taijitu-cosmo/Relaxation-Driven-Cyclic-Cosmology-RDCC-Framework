...
\subsection{Vorticity-Driven IGM Heating: The Lyman-\texorpdfstring{$\alpha$}{alpha} Turbulent Channel}
\label{sec:vorticity-igm}

As shown in Companion~XVIII, the relaxation-induced inter-sector vorticity acts as a
non-thermal heating channel for the low-density intergalactic medium (IGM). This mechanism
arises from the dimension-six operator that couples the stress tensors of the two sectors and
injects turbulent energy into the baryonic fluid on scales near the relaxation scale $k_{\mathrm{rel}}$.
The resulting dissipation modifies the thermal history of the IGM and leaves a measurable
imprint on the Lyman-$\alpha$ forest.

\subsubsection{Turbulent Heating Rate}

The vorticity-induced heating rate is given by
\begin{equation}
H_{\mathrm{vort}}(z)
=
...
\label{eq:vort-heating}
\end{equation}
where $\epsilon_{\mathrm{vort}} \sim 0.01$--$0.03$ parametrizes the efficiency of turbulent
dissipation. The exponential cutoff reflects the suppression of vorticity injection on scales
larger than the relaxation scale.

This heating channel is strongest in underdense regions, where shock heating is weak and
the turbulent cascade is more efficient.
...
T(\Delta,z)
=
T_{0}(z)\,\Delta^{\gamma(z)-1},
\end{equation}
by increasing the temperature of low-density regions. We write the RDCC-modified slope as
\begin{equation}
...
\label{eq:gamma-vort}
\end{equation}
with $\alpha_{\mathrm{vort}} \sim 0.05$--$0.1$.

Consequences:
\begin{itemize}
\item flatter $T$--$\Delta$ relation,
\item reduced temperature contrast between voids and filaments,
\item broader temperature distribution,
\item improved agreement with Lyman-$\alpha$ forest measurements at $z \sim 4$--$6$.
\end{itemize}

\subsubsection{Lyman-\texorpdfstring{$\alpha$}{alpha} Forest Constraints}
...
0.3\,h\,\mathrm{Mpc}^{-1},
\end{equation}
derived from the requirement that the heating does not over-flatten the observed
...
\item RSD velocity-dispersion constraints (Companion~XXVII),
\item 21-cm heating constraints (Section~4).
\end{itemize}

\subsubsection{Summary}

The vorticity-driven Lyman-$\alpha$ heating channel is a distinctive RDCC prediction. It:
\begin{itemize}
\item injects turbulent energy into the low-density IGM,
\item flattens the temperature--density relation,
\item delays the thermal peak of reionization,
\item and provides a direct observational probe of the relaxation scale $k_{\mathrm{rel}}$.
\end{itemize}
